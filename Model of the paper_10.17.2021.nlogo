...
to produce ;; S * ^SCAL*SKILL * ^PRO
  ask factories[
    ...
    set goods goods + ((money ^ 0.75 + 0.1) * ((pre-goods + 1) ^ 0.25) * (equipment + 1)) / 10
  ]
end
...
to pay-salary ;; K * SKILL//no
  ask factories [
    if (count in-job-neighbors with [breed = public]) > 0 [
      set salary  (money * 2 / 3) / ave-employee
...
to return-money ;; BALANCE
  ask banks [
    ...
    ask in-counter-neighbors with [breed = public] [
      let ower self
      set money money + 0.005 * owe
      ask out-counter-neighbors with [breed = banks] [
        set money money - 0.005 * [owe] of ower
      ]
    ]
  ]
...
to reset-equipment
  ask factories [
    if resetup = 0 [
      ...
      ifelse ( money / develop ) < [goods] of saller * [price] of saller
      [
        set equipment equipment + ( (money / develop) / [price] of saller) / 500
        ask saller [
          set goods goods - ([money] of buyer / develop) / price
          ;set real-money real-money + [money] of buyer
          set money money + ([money] of buyer / develop)
        ]
        ;set real-money real-money - money
        set money money - (money / develop)
      ]
      [
        set  equipment equipment + ([goods] of saller) / 500
  ...
  ask factories [
    ...
    (ifelse
      time = -2  [
        ...
        set inves-num (banks-money / count factories)
        if [money] of giver > inves-num [
          create-invesment-from giver
          ask in-invesment-neighbors with [breed = banks] [
            set money money - [inves-num] of ower
          ]
        set money money + inves-num
        set time 12
        set inves 0
        set owe 1.1 * inves-num
        set inves-num 0
        ;set resetup 1
        ]
      ]
    time = 0 [
      ifelse money * 0.5 >= owe [
        ask in-invesment-neighbors with [breed = banks] [
          set money money +  [owe] of ower
        ]
        set money money - owe
        set owe 0
        set time -2
      ...
      [
        ask in-invesment-neighbors with [breed = banks] [
          set money money + 0.5 * [money] of ower
        ]
        set owe owe - 0.5 * money
        set money 0.5 * money
        set time time + 1
      ]
    ]
...
to save-money
  ask banks [
    ...
    ask in-counter-neighbors with [breed = public] [
      ...
      set trans money + owe
       ask out-counter-neighbors with [breed = banks] [
        set money money + [trans] of giver * (1 / 3 ) - [owe] of giver
      ]
      set owe trans * (1 / 3 )
      set money trans * ( 1 - 1 / 3 )
    ]
  ]
end
...
to make-money
  ask banks [
    set money money + 0.025 * all-money / count banks
  ]
end
...
to-report all-real-money
  report public-money +  banks-money + factories-money
end
to-report all-money
  report public-money + banks-money + factories-money
end
